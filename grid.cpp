...
double Grid2D::kappa(double v, double lc, double lambda_x, double lambda_y) const
{
    ...
    case velocity_correlation_model::exponential_vdep: {
        ...
        double alpha_x = D * diffusionfactor_*lc / (v_safe * lambda_x * lambda_x);
        double alpha_y = D * diffusionfactor_*lc / (v_safe * lambda_y * lambda_y);
        return term_adv
...
void Grid2D::computeMixingDiffusionCoefficient()
{
    ...
    for (int j = 0; j <= ny_; ++j) {
        for (int i = 0; i < nx_; ++i) {
            ...
            // Compute u at this face: face j is at u = j/ny_
            double u = static_cast<double>(j) / static_cast<double>(ny_);
