...
#   - WSL is active by default (others commented out)
...
#   - Extra safety: fail fast if VTK vars are not set but GRID_USE_VTK is enabled
#   - Extra safety: add common system deps after VTK libs (-ldl -lpthread -lm)
#   - Extra safety: avoid /home/arash PETSc fallbacks on WSL
# ============================================================
...
contains(DEFINES, WSL) {
    ...
    # Fail fast if the install path is wrong
...
# Host-specific fallbacks (avoid accidental /home/arash on WSL)
contains(DEFINES, WSL) {
    isEmpty(PETSC_DIR)  { PETSC_DIR  = /home/behzad/petsc }
...
contains(DEFINES, WSL) {
    ...
    # Optional: prevent toolchain mixing in some QtCreator kits
    QMAKE_AR     = /usr/bin/ar
...
# ============================================================
GRID_USE_VTK {
    # Fail fast if VTK vars are not set
    isEmpty(VTKINSTALLPATH) | isEmpty(VTK_V) | isEmpty(VTK_INCNAME) {
        error("GRID_USE_VTK is enabled but VTK vars are not set. Check host DEFINES/CONFIG.")
    }
    ...
    # Common system deps (safe; helps avoid undefined refs on some VTK builds)
